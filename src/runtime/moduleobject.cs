    ...
    internal class CLRModule : ModuleObject
    {
        ...
        public static PyObject _load_clr_module(PyObject spec)
        {
            ...
            using (var modname = spec.GetAttr("name"))
            {
                ...
                // Runtime.XIncref(modname.Handle);
                // Runtime.PyTuple_SetItem(args.DangerousGetAddress(), 0, modname.Handle);
                // modptr = ImportHook.__import__(IntPtr.Zero, args.DangerousGetAddress(), IntPtr.Zero);
            }
            // return new PyObject(modptr)
